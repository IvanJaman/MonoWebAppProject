...
function App() {
  ...
  const handleSubmit = (e) => {
    ...
    if (!user.firstName || !user.lastName) {
      ...
      if (editId !== null) {
        axios
          .put(`https://localhost:7267/User/UpdateUser/${editId}`, user)
          .then(() => {
        ...
        axios
          .post("https://localhost:7267/User", user)
          .then((response) => {
      ...
      <header className="App-header">
        <div className="LoginDiv">
          ...
          <InputBar
            ...
            setInputValue={(value) => setUser({ ...user, firstName: value })}
            placeholder="First Name"
          />
          <InputBar
            inputValue={user.lastName}
            setInputValue={(value) => setUser({ ...user, lastName: value })}
